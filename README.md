...
├── ultra_sonic/          # 超音波センサー関連のモジュール
│   ├── __init__.py
│   └── distance.py       # 距離測定スクリプト
...
graph TD
    ...
    G1 --> H1[電子ペーパーへの写真表示]
    D1 -->|いいえ| D2{画像生成要求?}
    D2 --> |はい| E2[chat.py: DALL-E 3による画像生成]
    D2 --> |いいえ| F2[chat.py: GPT-4oによるテキスト応答]
    ...
    R --> A
    Q -- いいえ --> S{応答は長い?}
    S -- はい --> T[chat.py: 応答を要約]
    S -- いいえ --> I[epd_display.py: 電子ペーパーへのテキスト表示]
    T --> I
    ...
    M[ultra_sonic/distance.py: 距離測定] --> A
```
...
### 7. 超音波センサーによる自動起動
- 超音波センサーで一定距離内の物体を検知するとシステムを自動起動
- `ultra_sonic/distance.py` が `main.py` をバックグラウンドで実行
...
5. 超音波センサーによる自動起動:
   - センサーが一定距離内の物体を検知すると、システムが自動的に起動します。
